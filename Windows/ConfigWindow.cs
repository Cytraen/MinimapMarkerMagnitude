...
internal class ConfigWindow : Window, IDisposable
{
	...
	private short _iconPreviewSize = (short)(
		Math.Round(ImGuiHelpers.GlobalScaleSafe, MidpointRounding.AwayFromZero) * 64f
	);
	...
	private void DrawGroupEdit(int editGroup)
	{
		...
		using (var combo = ImRaii.Combo("##IconPreviewSelector", _iconPreviewSize + "x"))
		{
			if (combo.Success)
			{
				if (ImGui.Selectable(32 + "x"))
					_iconPreviewSize = 32;
				if (ImGui.Selectable(64 + "x"))
					_iconPreviewSize = 64;
				if (ImGui.Selectable(128 + "x"))
					_iconPreviewSize = 128;
				if (ImGui.Selectable(192 + "x"))
					_iconPreviewSize = 192;
				if (ImGui.Selectable(256 + "x"))
					_iconPreviewSize = 256;
			}
		}
		...
		using (
			var combo = ImRaii.Combo(
				"##Sorting",
				_sortIconsById ? "Internal Icon ID" : "Newest First"
			)
		)
		{
			if (combo.Success)
			{
				if (ImGui.Selectable("Newest First"))
					_sortIconsById = false;

				if (ImGui.Selectable("Internal Icon ID"))
					_sortIconsById = true;
			}
		}
		...
		)
		{
			if (child.Success)
			{
				...
				var itemsPerRow = (int)
					MathF.Floor(
						ImGui.GetContentRegionMax().X
							/ (_iconPreviewSize + 2 + ImGui.GetStyle().ItemSpacing.X)
					);

				var unassignedIcons = Services.SeenIcons.Where(x =>
					!Services
						.Config.IconGroups.SelectMany(y => y.GroupIconIds, (_, iconId) => iconId)
						.Contains(x) || group.GroupIconIds.Contains(x)
				);

				unassignedIcons = _sortIconsById
					? unassignedIcons.OrderBy(x => x)
					: unassignedIcons.Reverse();
				...
				foreach (var iconId in unassignedIcons)
				{
					...
					if (
						Services.CompiledSizeOverrides.ContainsKey(iconId)
						&& !group.GroupIconIds.Contains(iconId)
					)
						continue;
					...
					ImGui.Image(
						...
						new(1, 1),
						new(1, 1, 1, 1),
						selected ? new(0, 1, 0, 1) : new(1, 1, 1, 1)
					);
	...
	private void DrawMainConfig()
	{
		...
		var offMapIconScale = (float)(
			Math.Pow(Services.Config.DefaultOffMapIconScalar * 0.625f, 2f) * 100f
		);
		...
		using (ImRaii.PushFont(UiBuilder.IconFont))
		{
			if (
				...
				&& !string.IsNullOrWhiteSpace(_groupNameInput)
			)
			{
				Services.Config.IconGroups.Add(
					new MinimapIconGroup { GroupName = _groupNameInput }
				);
				_groupNameInput = string.Empty;
				changed = true;
			}
		...
		using (var table = ImRaii.Table("MinimapMarkerMagnitudeGroupTable", 4, tableFlags))
		{
			if (table.Success)
			{
				...
				ImGui.TableSetupColumn(
					...
					ImGui.CalcTextSize("Group Scale").X,
					1
				);
				ImGui.TableSetupColumn(
					"Enabled",
				...
				for (var i = 0; i < Services.Config.IconGroups.Count; i++)
				{
					...
					ImGui.Text(
						((float)(Math.Pow(group.GroupScale, 2f) * 100f)).ToString(
							@"###.#\%\%",
					...
					using (ImRaii.PushFont(UiBuilder.IconFont))
					{
						if (ImGui.Button(FontAwesomeIcon.Edit.ToIconString()))
							_currentEditingGroup = i;
					}
				}
			}
		}
